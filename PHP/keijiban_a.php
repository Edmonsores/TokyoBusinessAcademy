...
if (file_exists("keijiban_a.txt")) {
    $f = file("keijiban_a.txt"); // ファイルを配列に読み込む
...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // ボタンが押されたとき
    ...
    if (!$errMsg) { // エラーメッセージがない場合
        ...
        $data = array($no, $_POST['name'], $_POST['email'], $free, $time); // No, 名前, メッセージ, 時間を配列に
        $datag = implode(",", $data); // 配列をカンマ区切りで文字列に変換
        array_unshift($f, $datag . "\n"); // 配列$fの先頭にデータを追加

        $file = fopen("keijiban_a.txt", "w"); // 書き込み用でファイルを開く
        foreach ($f as $line) {
            fputs($file, $line); // ファイルに書き込み
        }
        fclose($file); // ファイルを閉じる

        header("Location:keijiban_a.php"); // リロード時の再投稿を防ぐ
        exit; // 終了
    }
}
?>
...
<body>
<form method="post" action="keijiban_a.php"> <!-- 自分自身を読み込む -->
<h1>掲示板</h1>
<?php
...
<tr>
    <td>
    名前: <input type="text" name="name" size="27"> メール: <input type="text" name="email" size="27"><br>
    <textarea name="free" cols="77" rows="8"></textarea><br><br>
...
foreach ($f as $line) { // ファイルから一行ずつ読み込み
    list($no, $name, $email, $free, $time) = explode(",", $line); // カンマで分割し変数に代入
